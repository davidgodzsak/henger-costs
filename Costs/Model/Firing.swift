import RealmSwift
import Foundation

class Firing: Object, ObjectKeyIdentifiable, Purchase {
    @Persisted(primaryKey: true) var _id: ObjectId
    @Persisted var fireType: FireType = FireType.BisqueAndHigh
    @Persisted var wasGlazed: Bool = false
    @Persisted var items: List<FiredItem>
    
    @Persisted var userId: ObjectId
    @Persisted var date: Date = Date()
    @Persisted var price: Int = 0
    @Persisted var isBilled: Bool = false
    @Persisted var markedDeleted: Bool = false
    
    // TODO: Move out of this function to a service or util
    static func priceForFiringItem(settings: Prices, wasGlazed: Bool, fireType: FireType, item: FiredItem) -> Int {
        let glaze = settings.glazePrice!
        // TODO: Hard to read
        let glazePrice =  wasGlazed ? Double(glaze.treshold) < item.size ? glaze.belowPrice : glaze.abovePrice : 0
        
        // TODO: ERROR HANDLING
        let exponentials = try! settings.firingPrice!.firingPriceSettingsFor(fireType)
        
        let firingPrice = exponentials
            .map { $0.A * exp($0.B * Double(item.size)) * item.size}
            .reduce(0, +)
        
        return Int(round(firingPrice)) + glazePrice
    }
    
    // TODO: Move out of this function to a service or util
    func priceForFirings(settings: Prices) -> Int {
        return items
            .map {Firing.priceForFiringItem(settings: settings, wasGlazed: self.wasGlazed, fireType: self.fireType, item: $0)}
            .reduce(0, +)
    }
}


class FiredItem: EmbeddedObject {
    @Persisted var size: Double = 0
    @Persisted var amount: Int = 0
    
    override var description: String {
        let s = size.rounded() == size ? String(Int(size)) : String(size)
        return amount > 1 ? "\(s)cm x \(amount)db" : "\(s)cm"
    }
}

enum FireType : String, CustomStringConvertible, PersistableEnum {
    case HighOnly, LowOnly, BisqueOnly, BisqueAndHigh, BisqueAndLow
    
    var description: String {
        switch self {
        case .HighOnly:
            return "csak magas"
        case .LowOnly:
            return "csak alacsony"
        case .BisqueOnly:
            return "csak zsenge"
        case .BisqueAndHigh:
            return "magas + zsenge"
        case .BisqueAndLow:
            return "alacsony + zsenge"
        }
    }
}


extension Firing {
    static let sampleData: [Firing] = [
        Firing(value:
                [
                    "date": Date(),
                    "fireType": FireTypeOld.BisqueAndLow,
                    "wasGlazed": false,
                    "items": [
                        FiredItem(value: ["size": 23, "amount": 1]),
                        FiredItem(value: ["size": 10, "amount": 5]),
                        FiredItem(value: ["size": 12, "amount": 1]),
                        FiredItem(value: ["size": 15, "amount": 2]),
                        FiredItem(value: ["size": 10, "amount": 3]),
                        FiredItem(value: ["size": 8, "amount": 3])
                    ],
                    "price": 4325
                ]
        )
    ]
}
